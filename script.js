...
function setupLoginEnhancements() {
  var activeLoginForm = document.querySelector("#loginForm");
  ...
  function validatePortalCredentials(form) {
    var portal = form.dataset.portal || "portal";
    var loginInput = form.querySelector("#loginId");
    var passwordInput = form.querySelector("#password");
    var loginId = loginInput ? loginInput.value.trim() : "";
    var password = passwordInput ? passwordInput.value : "";
    var rolePatterns = {
      admin: /^JGS-ADM-\d{3}$/i,
      teacher: /^JGS-FAC-\d{3}$/i,
      student: /^(22[A-Z0-9-]{5,}|[\w.-]+@[\w.-]+\.[A-Za-z]{2,})$/i,
      parent: /^JGS-PAR-\d{3}$/i
    };
    var examples = {
      admin: "Use format JGS-ADM-001.",
      teacher: "Use format JGS-FAC-001.",
      student: "Use your student ID or registered email.",
      parent: "Use format JGS-PAR-001."
    };
    var hasError = false;

    if (!loginId) {
      setFieldError(loginInput, "ID is required.");
      hasError = true;
    } else if (rolePatterns[portal] && !rolePatterns[portal].test(loginId)) {
      setFieldError(loginInput, examples[portal]);
      hasError = true;
    } else {
  ...
  if (activeLoginForm) {
    activeLoginForm.dataset.validationReady = "true";
    activeLoginForm.querySelectorAll("input").forEach(function (input) {
      input.addEventListener("input", function () {
        if (input.classList.contains("input-error")) setFieldError(input, "");
      });
    });
  }
  ...
  window.validatePortalCredentials = validatePortalCredentials;
}

function setupDashboardV11() {
  var dashboard = document.querySelector("#dashboardSection");
  if (!dashboard) return;
  ...
  function syncThemeButtons() {
    document.querySelectorAll("[data-dashboard-theme]").forEach(function (button) {
      button.textContent = document.body.classList.contains("dark-theme") ? "Light Mode" : "Dark Mode";
    });
  }
  ...
  document.querySelectorAll("[data-dashboard-theme]").forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      localStorage.setItem("jgs-theme", document.body.classList.contains("dark-theme") ? "dark" : "light");
      syncThemeButtons();
    });
  });

  document.querySelectorAll("[data-profile-menu-toggle]").forEach(function (button) {
    button.addEventListener("click", function () {
      var menu = button.closest(".admin-hero-card").querySelector("[data-profile-menu]");
      if (!menu) return;
  ...
  document.addEventListener("click", function (event) {
    if (event.target.closest("[data-profile-menu-toggle]") || event.target.closest("[data-profile-menu]")) return;
    document.querySelectorAll("[data-profile-menu].open").forEach(function (menu) {
      menu.classList.remove("open");
    });
  });

  var links = Array.prototype.slice.call(document.querySelectorAll("[data-dashboard-link]"));
...
var portalCredentials = {
  ...
  teacher: {},
  student: {
    "22JGS-AIML-001": atob("c3R1ZGVudDEyMw=="),
    "22A31A0501": atob("c3R1ZGVudDEyMw==")
  },
  parent: {
...
function renderTeacherDashboard(profile) {
  ...
  if (document.querySelector("#teacherProfileMenuName")) document.querySelector("#teacherProfileMenuName").textContent = profile.name;
  if (document.querySelector("#teacherProfileMenuId")) document.querySelector("#teacherProfileMenuId").textContent = profile.id + " / " + profile.subject;
...
function validatePortalLogin(portal, loginId, password) {
  ...
  if (portal === "unified") {
    return (
      validatePortalLogin("admin", id, pw) ||
      validatePortalLogin("teacher", id, pw) ||
      validatePortalLogin("student", id, pw) ||
      validatePortalLogin("parent", id, pw)
    );
  }
...
function renderStudentDashboard(profile) {
  ...
  if (document.querySelector("#studentProfileMenuName")) document.querySelector("#studentProfileMenuName").textContent = profile.name;
  if (document.querySelector("#studentProfileMenuId")) document.querySelector("#studentProfileMenuId").textContent = profile.id + " / " + profile.branch;
...
function setupTeacherFacultyLink() {
  var form = document.querySelector('#loginForm[data-portal="teacher"]');
  if (!form) return;
  var params = new URLSearchParams(window.location.search);
  var facultyId = params.get("faculty");
  if (!facultyId) return;
  var loginInput = document.querySelector("#loginId");
  var status = document.querySelector("#loginStatus");
  if (loginInput) loginInput.value = facultyId;
  if (status) status.textContent = "Faculty selected from directory. Enter the teacher password to open this dashboard.";
}
...
if (loginForm) {
  loginForm.addEventListener("submit", function (event) {
    ...
    if (window.validatePortalCredentials && !window.validatePortalCredentials(loginForm)) {
      if (status) status.textContent = "Please fix the highlighted login details.";
      return;
    }

    var localCredentialValid = validatePortalLogin(portal, normalizedLoginId, password);

    if (isLoginLocked(portal, normalizedLoginId) && !localCredentialValid) {
    ...
    apiRequest("/api/login", {
      method: "POST",
      body: JSON.stringify({ loginId: normalizedLoginId, password: password })
    }).then(function (response) {
      var actualPortal = response.portal || portal;
      loginForm.classList.remove("is-loading");
      recordSuccessfulLogin(actualPortal, normalizedLoginId);
      savePortalSession(actualPortal, normalizedLoginId);
      if (status) status.textContent = "Login successful. Opening " + actualPortal + " dashboard.";
      setTimeout(function () {
        openPortalDashboard(actualPortal, normalizedLoginId, false);
      }, 450);
    }).catch(function () {
      loginForm.classList.remove("is-loading");
      if (localCredentialValid) {
        var fallbackPortal = portal === "unified" ? detectPortalFromLoginId(normalizedLoginId) : portal;
        recordSuccessfulLogin(fallbackPortal, normalizedLoginId);
        savePortalSession(fallbackPortal, normalizedLoginId);
        if (status) status.textContent = "Login successful. Opening " + fallbackPortal + " dashboard.";
        setTimeout(function () {
          openPortalDashboard(fallbackPortal, normalizedLoginId, false);
        }, 250);
        return;
      }
      var attempt = recordFailedLogin(portal, normalizedLoginId);
      if (status) {
...
function detectPortalFromLoginId(loginId) {
  var id = (loginId || "").trim();
  if (/^JGS-ADM-/i.test(id)) return "admin";
  if (/^JGS-FAC-/i.test(id)) return "teacher";
  if (/^JGS-PAR-/i.test(id)) return "parent";
  return "student";
}
